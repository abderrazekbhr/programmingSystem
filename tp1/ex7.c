...
int main() {
    ...
    while (1) {
        ...
        // parent process
            ...
            long elapsed_time =(t_finish_child.tv_nsec - t_init_child.tv_nsec) / 1000000;

            char msgState[100] = "\0";

            // test if the process completed correctly or if there is an interruption
            if (WIFEXITED(status)) {
                sprintf(msgState, "enseash %% [exit : %d | %ldms]\n", WEXITSTATUS(status), elapsed_time);
            } else {
                sprintf(msgState, "enseash %% [sign : %d | %ldms]\n", status, -elapsed_time);
            }
